...
export default function MapView({ segments = [], gpsTrack = null, height = '400px', className = '' }) {
  const allPositions = [
    ...segments.flatMap(s => s.polyline || []),
    ...(gpsTrack || []),
  ]

  const center = allPositions.length > 0
    ? [allPositions[0].lat, allPositions[0].lng]
    : [50.45, 30.52] // Kyiv as default

  return (
    <MapContainer center={center} zoom={14} style={{ height, width: '100%', borderRadius: '8px' }} className={className}>
      <TileLayer
      ...
      {segments.map((segment, i) => {
        if (!segment.start_point) return null
        return (
          <Marker key={`start-${segment.id || i}`} position={[segment.start_point.lat, segment.start_point.lng]} icon={startIcon}>
      ...
      {segments.map((segment, i) => {
        if (!segment.end_point) return null
        return (
          <Marker key={`end-${segment.id || i}`} position={[segment.end_point.lat, segment.end_point.lng]} icon={endIcon}>
