  ...
  return (
    <div style={{ maxWidth: '400px', margin: '4rem auto', padding: '2rem', border: '1px solid #ddd', borderRadius: '8px' }}>
      ...
      <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
        ...
        <button type="submit" disabled={loading} style={btnStyle}>{loading ? '...' : t(`auth.${mode}`)}</button>
      </form>

      <div style={{ marginTop: '1rem', textAlign: 'center' }}>
        <p style={{ color: '#666', marginBottom: '0.5rem' }}>{t('auth.orContinueWith')}</p>
        <a href="/auth/google_oauth2" style={{ ...btnStyle, display: 'inline-block', background: '#4285f4', textDecoration: 'none', color: '#fff' }}>Google</a>
        {' '}
        <a href="/auth/apple"         style={{ ...btnStyle, display: 'inline-block', background: '#000',    textDecoration: 'none', color: '#fff' }}>Apple</a>
        {' '}
        <a href="/auth/strava"        style={{ ...btnStyle, display: 'inline-block', background: '#fc4c02', textDecoration: 'none', color: '#fff' }}>Strava</a>
      </div>

      <p style={{ textAlign: 'center', marginTop: '1rem' }}>
        {mode === 'login' ? t('auth.noAccount') : t('auth.hasAccount')}{' '}
        <button onClick={() => setMode(mode === 'login' ? 'register' : 'login')} style={{ background: 'none', border: 'none', color: '#4285f4', cursor: 'pointer' }}>
...
const btnStyle   = { padding: '0.6rem 1.2rem', background: '#1a1a2e', color: '#fff', border: 'none', borderRadius: '4px', cursor: 'pointer', fontSize: '1rem' }
